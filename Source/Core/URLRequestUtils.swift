        ...
        if let logger = logger {
            ...
            if let response = (self as? HTTPURLResponse) {
                for headerKey in response.allHeaderFields.keys {
                    logger.d("[REQUEST] Key: \(headerKey), Value: \(String(describing: response.allHeaderFields[headerKey]))")
                }
            }
...
class URLRequestUtils {
    ...
    static func makeRequest(_ url:URL,
                            ...
                            parameters:Any? = nil) -> URLRequest {
        let postUrl: URL
        var body: Data? = nil
        if methodType != .get {
            postUrl = url
            body = requestSerializer.encode(parameters)
        } else {
            // url append
            if let serializer: QueryRequestSerializer = requestSerializer as? QueryRequestSerializer {
                let queries: [URLQueryItem]? = serializer.encode(parameters)
        ...
        if let contentType = requestSerializer.contentType, request.allHTTPHeaderFields != nil {
            request.allHTTPHeaderFields?.updateValue(contentType, forKey: "Content-Type")
        }
