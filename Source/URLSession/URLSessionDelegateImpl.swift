...
extension URLSessionDelegateImpl: URLSessionTaskDelegate {
    ...
    func urlSession(_ session: URLSession,
        ...
        HttpMessengerSession.logger?.d("a")
        let interceptResult = executeIntercepter(task, data: self.data, task.response, (error != nil ? .networkingError(from: error!) : nil))
        // 丁寧に
    ...
    func urlSession(_ session: URLSession,
        ...
        if let response = dataTask.response {
            executeResponseProgress(dataTask,
                                    ...
                                    totalBytesReceived: Int64(self.data!.count),
